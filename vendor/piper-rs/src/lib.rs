...
impl VitsStreamingModel {
    ...
    fn infer_with_values(&self, input_phonemes: Vec<i64>) -> PiperAudioResult {
        ...
        let audio = encoder_output.infer_decoder(&self.decoder_model)?;
        let inference_ms = timer.elapsed().as_millis() as f32;
        Ok(Audio::new(
            audio,
            self.config.audio.sample_rate as usize,
    ...
    fn infer_encoder(&self, input_phonemes: Vec<i64>) -> PiperResult<EncoderOutputs> {
        ...
        let result = match session.run(SessionInputs::from(inputs.as_slice())) {
...
impl EncoderOutputs {
    ...
    fn infer_decoder(&self, session: &Mutex<Session>) -> PiperResult<AudioSamples> {
        let mut inputs = vec![
        ...
        let result = match outputs[0].try_extract_tensor::<f32>() {
            Ok((_shape, data)) => Ok(data.to_vec().into()),
            Err(e) => Err(PiperError::OperationError(format!(
                "Failed to run model inference. Error: {}",
                e
            ))),
...
impl SpeechStreamer {
    ...
    fn synthesize_chunk(
        ...
        let audio = {
            ...
            drop(outputs);
            drop(session);
            self.process_chunk_audio(audio_arr.view(), audio_index)?
        };
        Ok(audio)
    }
    #[inline(always)]
