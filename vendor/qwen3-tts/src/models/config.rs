...
/// Quantization configuration for memory-efficient inference
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QuantizationConfig {
    /// Enable quantization
    pub enabled: bool,
    /// Quantization dtype: "int8" or "int4"
    pub dtype: QuantizationDtype,
    /// Layers to quantize ("all", "attention", "mlp")
    pub layers: QuantizationLayers,
}
...
impl Default for QuantizationConfig {
    fn default() -> Self {
        Self {
            ...
            layers: QuantizationLayers::All,
        }
    }
}

impl QuantizationConfig {
    /// Create config for Raspberry Pi deployment (INT8, all layers)
    pub fn for_raspberry_pi() -> Self {
        Self {
            enabled: true,
            dtype: QuantizationDtype::Int8,
            layers: QuantizationLayers::All,
    ...
    pub fn memory_savings_percent(&self) -> f32 {
        ...
        let dtype_savings = match self.dtype {
            QuantizationDtype::Int8 => 0.5,  // 50% reduction vs BF16
            QuantizationDtype::Int4 => 0.75, // 75% reduction vs BF16
        };

        let layer_coverage = match self.layers {
            QuantizationLayers::All => 1.0,       // 100% of model
            QuantizationLayers::Attention => 0.6, // ~60% of parameters
            QuantizationLayers::Mlp => 0.4,       // ~40% of parameters
        };

        dtype_savings * layer_coverage * 100.0
    }
}
...
mod tests {
    ...
    fn test_quantization_config_default() {
        ...
        assert_eq!(config.layers, QuantizationLayers::All);
        assert_eq!(config.memory_savings_percent(), 0.0);
    }

    #[test]
    #[cfg(feature = "quantized")]
    fn test_quantization_config_for_raspberry_pi() {
        let config = QuantizationConfig::for_raspberry_pi();
        assert!(config.enabled);
        assert_eq!(config.dtype, QuantizationDtype::Int8);
        assert_eq!(config.layers, QuantizationLayers::All);
        assert_eq!(config.memory_savings_percent(), 50.0);
    }
    ...
    fn test_quantization_memory_savings() {
        let config = QuantizationConfig {
            enabled: true,
            dtype: QuantizationDtype::Int8,
            layers: QuantizationLayers::Attention,
        };
        // INT8 = 50% savings, Attention = 60% coverage
        // 0.5 * 0.6 * 100 = 30%
        let savings = config.memory_savings_percent();
        assert!((savings - 30.0).abs() < 1e-5);
        ...
        // INT4 = 75% savings, All = 100% coverage
        let savings_int4 = config_int4.memory_savings_percent();
        assert!((savings_int4 - 75.0).abs() < 1e-5);
    }
