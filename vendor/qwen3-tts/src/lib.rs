...
impl Qwen3TTS {
    ...
    pub fn synthesize_streaming(
        ...
        let input_ids = self.text_tokenizer.encode(text)?;
        let options = options.with_speaker_safety_limits(speaker, self.model_type.as_ref());
...
impl SynthesisOptions {
    ...
    /// Apply speaker-specific safety limits to max_length.
    ///
    /// Ryan speaker (ID 3061) has EOS generation issues in 0.6B CustomVoice models,
    /// leading to very long outputs. This applies a conservative 512 frame limit
    /// (40 seconds) for Ryan to prevent runaway generation.
    pub fn with_speaker_safety_limits(
        mut self,
        speaker: Speaker,
        model_type: Option<&ModelType>,
    ) -> Self {
        // Only apply limits for Ryan speaker in CustomVoice models
        if speaker == Speaker::Ryan {
            if let Some(ModelType::CustomVoice) = model_type {
                // Limit Ryan to 40s max (512 frames at 12.5 Hz = ~40.96s)
                if self.max_length > 512 {
                    tracing::debug!(
                        "Applying safety limit for Ryan speaker: {} -> 512 frames (40s max)",
                        self.max_length
                    );
                    self.max_length = 512;
                }
...
mod tests {
    ...
    #[test]
    fn test_synthesis_options_ryan_safety_limit() {
        use models::talker::Speaker;
        use models::ModelType;

        // Ryan speaker in CustomVoice model should be limited to 512 frames
        let options = SynthesisOptions {
            max_length: 2048,
            ..Default::default()
        };
        ...
        let limited =
            options.with_speaker_safety_limits(Speaker::Ryan, Some(&ModelType::CustomVoice));
        assert_eq!(
            limited.max_length, 512,
            "Ryan speaker should be limited to 512 frames (40s)"
        );

        // Ryan in Base model should not be limited
        let options_base = SynthesisOptions {
            max_length: 2048,
            ..Default::default()
        };
        let not_limited =
            options_base.with_speaker_safety_limits(Speaker::Ryan, Some(&ModelType::Base));
        assert_eq!(
            not_limited.max_length, 2048,
            "Ryan in Base model should not be limited"
        );

        // Other speakers should not be limited
        let options_aiden = SynthesisOptions {
            max_length: 2048,
            ..Default::default()
        };
        let aiden_not_limited =
            options_aiden.with_speaker_safety_limits(Speaker::Aiden, Some(&ModelType::CustomVoice));
        assert_eq!(
            aiden_not_limited.max_length, 2048,
            "Aiden speaker should not be limited"
        );
    }
